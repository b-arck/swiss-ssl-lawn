...
Catalyst Model that load file with all data
...
sub loadDetails{
	...
	foreach my $element (values($perl)) {
		bless $element, 'Survey';
		$id = $element->get_id();
		$site = $element->get_hostName();
		if( $id == $fields[2] and $site eq $fields[3] ){
			$xmlData->{$i} = $element;
		}	
		$i++;
	}
...
sub loadSiteByType{
	...
	foreach my $element (values($perl)) {
		bless $element, 'Survey';
		if($fields[1] ne "all"){
			$hosttype = $element->get_hostType();
			if( $hosttype eq $fields[1]){
				$xmlData->{$i} = $element;
			}	
			$i++;
		}
